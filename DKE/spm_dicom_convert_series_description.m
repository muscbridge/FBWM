...
for i=1:length(hdr),
    if ~checkfields(hdr{i},'Modality') || ~(strcmp(hdr{i}.Modality,'MR') ||...
        ...
        guff = [guff(:)',hdr(i)];
    elseif ~checkfields(hdr{i},'StartOfPixelData','SamplesPerPixel',... %% to SamplesPerPixel
...
if hdr.SamplesPerPixel ~= 1,
    warning([hdr.Filename ': SamplesPerPixel = ' num2str(hdr.SamplesPerPixel) ' - cant be an MRI']); %to SamplesPerPixel
    return;
end;
